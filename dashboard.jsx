...
// ── Real Data as of April 2, 2026 close ──
const MOCK = {
  // 估值與基本面
  buffett: { value: 205.3, prev: 210.8, threshold: { green: 120, yellow: 160, red: 200 }, unit: "%", name: "巴菲特指標", sub: "Wilshire 5000 / GDP" },
  cape: { value: 32.5, prev: 33.8, threshold: { green: 20, yellow: 25, red: 30 }, unit: "x", name: "席勒本益比", sub: "Shiller CAPE Ratio" },
  // 恐慌情緒
  vix: { value: 24.54, prev: 25.52, threshold: { green: 20, yellow: 30, red: 40 }, unit: "", name: "VIX 恐慌指數", sub: "S&P 500 隱含波動率" },
  fearGreed: { value: 18, prev: 22, threshold: { green: 50, yellow: 30, red: 15 }, unit: "", name: "恐懼與貪婪", sub: "CNN Fear & Greed", invertColor: true },
  putCall: { value: 1.05, prev: 0.95, threshold: { green: 0.7, yellow: 0.9, red: 1.2 }, unit: "", name: "Put/Call Ratio", sub: "CBOE 選擇權比率" },
  // 流動性
  sofr_iorb: { value: 2.8, prev: 1.5, threshold: { green: 0, yellow: 1, red: 3 }, unit: "bps", name: "SOFR − IORB", sub: "短端資金壓力" },
  onrrp: { value: 148, prev: 185, threshold: { green: 300, yellow: 200, red: 150 }, unit: "B", name: "ON RRP 餘額", sub: "流動性緩衝墊", invertColor: true },
  tga: { value: 720, prev: 680, threshold: { green: 400, yellow: 600, red: 750 }, unit: "B", name: "TGA 餘額", sub: "財政部帳戶" },
  srf: { value: 2.1, prev: 0.8, threshold: { green: 0, yellow: 1, red: 5 }, unit: "B", name: "SRF 使用量", sub: "Fed 後備工具" },
  // 債券與利率
  us10y: { value: 4.31, prev: 4.32, threshold: { green: 3.5, yellow: 4.0, red: 4.5 }, unit: "%", name: "美債 10Y 殖利率", sub: "US Treasury 10Y" },
  us2y: { value: 3.79, prev: 3.80, threshold: { green: 3.0, yellow: 3.5, red: 4.0 }, unit: "%", name: "美債 2Y 殖利率", sub: "US Treasury 2Y" },
  spread2s10s: { value: 0.52, prev: 0.52, threshold: { green: 0.5, yellow: 0.2, red: 0 }, unit: "%", name: "2Y-10Y 利差", sub: "殖利率曲線斜率", invertColor: true },
  breakeven: { value: 2.68, prev: 2.55, threshold: { green: 2.0, yellow: 2.5, red: 3.0 }, unit: "%", name: "10Y 通膨預期", sub: "Breakeven Inflation" },
  move: { value: 115, prev: 108, threshold: { green: 80, yellow: 100, red: 120 }, unit: "", name: "MOVE Index", sub: "美債波動率" },
  // 信用與金融狀況
  hySpread: { value: 346, prev: 330, threshold: { green: 300, yellow: 400, red: 500 }, unit: "bps", name: "高收益債利差", sub: "HY OAS Spread" },
  bloomberg: { value: -0.72, prev: -0.55, threshold: { green: 0, yellow: -0.5, red: -1.0 }, unit: "", name: "彭博金融狀況", sub: "Bloomberg FCI", invertColor: true },
  // 避險與商品
  gold: { value: 4623, prev: 4750, threshold: { green: 4000, yellow: 4800, red: 5200 }, unit: "$", name: "黃金現貨", sub: "XAU/USD" },
  wti: { value: 111.54, prev: 100.12, threshold: { green: 70, yellow: 85, red: 100 }, unit: "$", name: "WTI 原油現金期貨", sub: "CL1 Front Month" },
  brent: { value: 109.03, prev: 101.16, threshold: { green: 75, yellow: 90, red: 105 }, unit: "$", name: "布蘭特原油現金期貨", sub: "CO1 Front Month" },
  dxy: { value: 100.13, prev: 99.5, threshold: { green: 100, yellow: 103, red: 106 }, unit: "", name: "美元指數", sub: "DXY Index" },
  // 大盤
  sp500: { value: 6582.69, prev: 6575.32, name: "S&P 500", sub: "標普500指數" },
  nasdaq: { value: 21879.18, prev: 21839.45, name: "NASDAQ", sub: "那斯達克綜合" },
  dow: { value: 46504.67, prev: 46565.74, name: "道瓊工業", sub: "Dow Jones" },
  sox: { value: 7478.76, prev: 7249.12, name: "費城半導體", sub: "PHLX SOX" },
};
...
const HISTORY_VIX = [17, 18, 20, 22, 25, 30, 35, 42, 38, 33, 30, 28, 32, 36, 29, 27, 25, 28, 25.5, 24.5];
const HISTORY_SP = [6920, 6880, 6820, 6750, 6650, 6530, 6420, 6350, 6400, 6480, 6520, 6560, 6500, 6450, 6510, 6540, 6575, 6560, 6575, 6583];
...
function MetricCard({ id, data, showGauge }) {
  ...
  return (
    ...
    >
      ...
      {data.threshold && (() => {
        const { green, red } = data.threshold;
        const inv = data.invertColor;
        // For normal: green=safe(low), red=danger(high). Bar goes 0%(green) → 100%(red)
        // For inverted: green=safe(high), red=danger(low). Bar goes 0%(green) → 100%(red)
        const lo = inv ? red : green;
        const hi = inv ? green : red;
        const rangeMin = Math.min(lo, hi);
        const rangeMax = Math.max(lo, hi);
        const pctRaw = rangeMax === rangeMin ? 50 : ((data.value - rangeMin) / (rangeMax - rangeMin)) * 100;
        const pctBar = inv ? (100 - Math.min(100, Math.max(0, pctRaw))) : Math.min(100, Math.max(0, pctRaw));
        const leftLabel = inv ? hi : lo;
        const rightLabel = inv ? lo : hi;
        const fmt = (v) => Math.abs(v) >= 100 ? v.toFixed(0) : v % 1 === 0 ? v.toString() : v.toFixed(1);
        return (
          <div style={{ marginTop: 10 }}>
            <div style={{ height: 4, background: "#0f172a", borderRadius: 2, overflow: "hidden", position: "relative" }}>
              <div style={{
                position: "absolute", left: 0, top: 0, height: "100%", borderRadius: 2,
                width: `${Math.min(100, Math.max(3, pctBar))}%`,
                background: `linear-gradient(90deg, ${colors[signal]}90, ${colors[signal]})`,
                transition: "width 0.5s ease",
              }} />
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
              <span style={{ fontSize: 9, color: "#22c55e", fontFamily: "'JetBrains Mono', monospace" }}>{fmt(leftLabel)}</span>
              <span style={{ fontSize: 9, color: "#ef4444", fontFamily: "'JetBrains Mono', monospace" }}>{fmt(rightLabel)}</span>
            </div>
          </div>
        );
      })()}
...
// ── Bottom Checklist (7-Day Rolling Verification) ──
const CHECKLIST_7D = {
  date: "2026-04-02",
  window: "03/25 → 04/02",
  items: [
    { label: "巴菲特指標回落至合理區間", met: false, current: "205.3%", range7d: "200.8–205.3%", target: "< 160%", verdict: "7天最低200.8%，遠高於160%" },
    { label: "CAPE 回落至 20-25x", met: false, current: "32.5x", range7d: "31.5–32.5x", target: "20-25x", verdict: "7天最低31.5x，仍遠高於25x" },
    { label: "VIX 噴出後底背離", met: true, current: "24.54", range7d: "盤高42.1→24.5", target: "> 40 噴出後背離", verdict: "3/25 VIX觸42.1噴出，3/27 SP新低但VIX僅36.8，底背離成立" },
    { label: "恐懼與貪婪指數極度恐懼", met: true, current: "18", range7d: "最低11，4天<15", target: "≤ 15 且維持", verdict: "7天內4天低於15（最低11），極度恐懼已確認" },
    { label: "Put/Call Ratio 極端悲觀", met: false, current: "1.05", range7d: "峰值1.42", target: "≥ 1.5 連續數日", verdict: "峰值1.42，3天>1.2但未達1.5門檻" },
    { label: "巨量換手 + 縮量止跌", met: true, current: "98M", range7d: "162M→98M", target: "放量崩跌→縮量止跌", verdict: "3/27巨量162M崩跌，後4天縮量98M且股價回升，型態成立" },
    { label: "高收益債利差見頂回落", met: true, current: "346 bps", range7d: "395→346 bps", target: "峰值回落>10%", verdict: "從3/27峰值395回落至346（-12.4%），信用壓力緩解" },
    { label: "VIX 快速回落確認賣壓結束", met: true, current: "24.54", range7d: "35.2→24.5", target: "峰值回落至 < 25", verdict: "從35.2回落至24.54（-30%），已跌破25，被動賣壓結束" },
  ],
};

function BottomChecklist() {
  const checks = CHECKLIST_7D.items;
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, color: "#64748b", fontFamily: "'JetBrains Mono', monospace", marginBottom: 12 }}>
        7日滾動驗證窗口：{CHECKLIST_7D.window} ・ 資料日期：{CHECKLIST_7D.date}
      </div>
      {checks.map((c, i) => (
        <div key={i} style={{
          padding: "8px 10px", marginBottom: 4, borderRadius: 6,
          background: c.met ? "#22c55e08" : "transparent",
          cursor: "pointer",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{
              ...
              fontSize: 12, color: c.met ? "#22c55e" : "#475569", flexShrink: 0,
            }}>
              {c.met ? "✓" : ""}
            </span>
            <div style={{ flex: 1 }}>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ fontSize: 9, color: "#475569", fontFamily: "'JetBrains Mono', monospace" }}>7d: {c.range7d}</div>
            </div>
            <span style={{ fontSize: 10, color: "#475569", marginLeft: 4 }}>{expanded === i ? "▾" : "▸"}</span>
          </div>
          {expanded === i && (
            <div style={{
              ...
              <div><span style={{ color: "#64748b" }}>門檻：</span>{c.target}</div>
              <div><span style={{ color: "#64748b" }}>判定：</span><span style={{ color: c.met ? "#22c55e" : "#ef4444" }}>{c.verdict}</span></div>
            </div>
          )}
        </div>
      ))}
    ...
    <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: "#64748b", fontFamily: "'JetBrains Mono', monospace" }}>
                US MARKET WAR ROOM ・ DATA AS OF 2026/04/02 CLOSE
              </div>
            </div>
          </div>
      ...
      <div style={{ textAlign: "center", padding: "24px 0", borderTop: "1px solid #1e293b", marginTop: 20 }}>
        <div style={{ fontSize: 11, color: "#475569", fontFamily: "'JetBrains Mono', monospace" }}>
          ⚠ 數據截至 2026/04/02 收盤 ・ 非即時報價 ・ 非投資建議 ・ 資料來源：大叔美股筆記 + 公開市場數據
        </div>
      </div>
    </div>
  );
